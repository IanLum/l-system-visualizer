...
Looking at the implementation of `Dragon.visualize`, it takes three arguments (other than `self`):
- `steps`: An integer representing the number of steps to generate, an then visualize.
- `size`: An optional integer, representing the size of each "forward" segment in the drawing, defaults to `3`.
- `show_drawing`: An optional boolean, representing whether the drawing animation is shown or not, defaults to `true`.
